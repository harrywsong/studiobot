...
class LotteryCog(commands.Cog):
    ...
    def create_lottery_interface_embed(self) -> discord.Embed:
        """Create the main lottery interface embed with enhanced information"""
        # Get lottery data for the main guild
        main_guild_id = None
        ...
        embed = discord.Embed(
            title="🎰 복권 시스템",
            description="크래시 게임 수수료와 패배 기여금으로 쌓인 복권 팟에 참가하세요!\n아래 버튼을 눌러 1-35 중 5개 번호를 선택하세요.",
    ...
    async def update_lottery_interface(self):
        """Update the lottery interface embed with current data and enhanced error handling"""
        ...
        try:
            ...
            self.logger.debug("복권 인터페이스가 성공적으로 업데이트되었습니다.")

        except discord.NotFound:
            self.logger.warning("복권 인터페이스 메시지를 찾을 수 없어 새로 생성합니다.")
            await self.setup_lottery_interface()
        except discord.HTTPException as e:
            self.logger.error(f"복권 인터페이스 업데이트 실패: {e}")
        except Exception as e:
            self.logger.error(f"복권 인터페이스 업데이트 중 예상치 못한 오류: {e}")
    ...
    async def add_to_pot(self, guild_id: int, amount: int):
        ...
        try:
            ...
            # Auto-update interface whenever pot changes
